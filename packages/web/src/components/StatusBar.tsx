...
export function StatusBar({ view }: { view: HealthView }): React.JSX.Element {
  ...
  if (conn === "disconnected") {
    return (
      ...
      >
        ...
        <span className="sr-only rail:not-sr-only rail:group-data-[collapsed=true]/shell:sr-only">Disconnected from laptop API — data may be unavailable.</span>
      </div>
    );
  }
  ...
  return (
    <div
      ...
      aria-label="connection status"
      data-conn={conn}
      className="flex flex-col gap-1 items-center rail:items-start rail:group-data-[collapsed=true]/shell:items-center px-2 rail:px-3 py-2 text-xs text-zinc-600 dark:text-zinc-300"
    >
      <span className="flex items-center gap-1.5">
        <span className={`inline-block w-1.5 h-1.5 rounded-full ${dotClass}`} aria-hidden="true" />
        <span className="sr-only rail:not-sr-only rail:group-data-[collapsed=true]/shell:sr-only">{loopAlive ? "sync loop live" : "loop down"}</span>
      </span>
      <span className="text-zinc-500 dark:text-zinc-400 tabular-nums sr-only rail:not-sr-only rail:group-data-[collapsed=true]/shell:sr-only">
        last pull {health?.lastPullAgeSec != null ? `${health.lastPullAgeSec}s ago` : "—"}
      </span>
      <span className="mono text-[11px] text-zinc-400 sr-only rail:not-sr-only rail:group-data-[collapsed=true]/shell:sr-only">{loopLabel(health?.jwtState ?? "unknown")}</span>

      {conn === "stale" && (
        <span
          data-stale="true"
          className="flex items-start gap-1.5 leading-relaxed text-amber-700 dark:text-amber-400 sr-only rail:not-sr-only rail:group-data-[collapsed=true]/shell:sr-only"
        >
